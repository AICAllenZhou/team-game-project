...
function publicPlayer(p){const {id,name,x,y,z,yaw,pitch,hp,ammo,kills,deaths,reloadUntil,deadUntil,color}=p;return {id,name,x,y,z,yaw,pitch,hp,ammo,kills,deaths,reloadUntil,deadUntil,color};}
...
const server=http.createServer(async(req,res)=>{
 ...
 if(req.method==='POST'&&url.pathname.startsWith('/api/')){
  ...
  if(url.pathname==='/api/join'){
   ...
   const p={id:randomUUID(),token:randomUUID(),room:key,name:String(data.name||'Drifter').slice(0,16),...spawn(),yaw:0,pitch:0,hp:100,ammo:6,kills:0,deaths:0,reloadUntil:0,deadUntil:0,lastShot:0,lastSeen:Date.now(),input:{},color:[0xc17a47,0x658c91,0x96799c,0x879466][room.size%4]};room.set(p.id,p);sessions.set(p.token,p);
  ...
  if(url.pathname==='/api/input'){p.input={x:data.x,z:data.z,yaw:data.yaw,pitch:data.pitch,jump:!!data.jump};}
  else if(url.pathname==='/api/reload'&&p.hp>0&&!p.reloadUntil&&p.ammo<6)p.reloadUntil=now+1800;
  else if(url.pathname==='/api/fire'&&p.hp>0&&!p.reloadUntil&&p.ammo>0&&now-p.lastShot>=420){
   p.lastShot=now;p.ammo--;const origin={x:p.x,y:p.y+1.5,z:p.z},direction={x:-Math.sin(p.yaw)*Math.cos(p.pitch),y:Math.sin(p.pitch),z:-Math.cos(p.yaw)*Math.cos(p.pitch)};
