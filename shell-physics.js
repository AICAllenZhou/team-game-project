...
export function createShellPhysics(scene,capacity=128){
 ...
 const shells=Array.from({length:capacity},()=>({position:new THREE.Vector3(),velocity:new THREE.Vector3(),rotation:new THREE.Quaternion(),spin:new THREE.Vector3(),size:1,age:0,awake:false}));
 ...
 function eject(frame,x,inheritedVelocity){
  ...
  shell.spin.set(5+Math.random()*4,(Math.random()-.5)*8,Math.sign(x)*3);shell.age=0;shell.awake=true;dirty=true;
 }
 function step(h){
  for(let i=0;i<count;i++){const s=shells[i];if(!s.awake)continue;dirty=true;s.age+=h;s.velocity.y-=9.81*h;s.position.addScaledVector(s.velocity,h);
 ...
 function update(dt){
  ...
  if(!dirty)return;
  for(let i=0;i<count;i++){const s=shells[i];matrix.compose(s.position,s.rotation,scale.setScalar(s.size));body.setMatrixAt(i,matrix);caps.setMatrixAt(i,matrix);}
  body.count=caps.count=count;body.instanceMatrix.needsUpdate=true;caps.instanceMatrix.needsUpdate=true;dirty=false;
 }
