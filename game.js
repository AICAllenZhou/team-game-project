...
const flash=mesh(new THREE.ConeGeometry(.08,.27,5),new THREE.MeshBasicMaterial({color:0xffd279}),rig,.3,-.185,-1.23);flash.rotation.x=-Math.PI/2;flash.visible=false;
...
let token=null,id=null,events=null,online=false,joining=false,local={x:0,y:0,z:8,hp:100,ammo:6},yaw=0,pitch=0,recoil=0,lastShot=0,reloading=0,vy=0,last=performance.now(),started=last,serverTime=0,receivedAt=0;
const keys=new Set(),peers=new Map(),traces=[];let audio;
function sound(){try{audio??=new AudioContext();audio.resume();const n=audio.createBufferSource(),b=audio.createBuffer(1,audio.sampleRate*.12,audio.sampleRate),a=b.getChannelData(0);for(let i=0;i<a.length;i++)a[i]=(Math.random()*2-1)*(1-i/a.length)**3;n.buffer=b;const filter=audio.createBiquadFilter();filter.type='lowpass';filter.frequency.value=1500;const gain=audio.createGain();gain.gain.value=.15;n.connect(filter).connect(gain).connect(audio.destination);n.start();}catch{}}
...
function shotEffect(s){const origin=new THREE.Vector3(s.origin.x,s.origin.y,s.origin.z),end=origin.clone().addScaledVector(new THREE.Vector3(s.direction.x,s.direction.y,s.direction.z),s.distance);const line=new THREE.Line(new THREE.BufferGeometry().setFromPoints([origin,end]),new THREE.LineBasicMaterial({color:0xffe1a1,transparent:true,opacity:.8}));scene.add(line);traces.push({line,life:.08});if(s.id===id&&s.hit){$('hit').style.opacity=1;setTimeout(()=>$('hit').style.opacity=0,130);}}
function applyState(s){serverTime=s.time;receivedAt=performance.now();const mine=s.players.find(p=>p.id===id);if(!mine)return;local=mine;$('health').textContent=mine.hp;$('ammo').textContent=mine.ammo;$('connection').textContent=`${s.players.length} / 12 • ${$('room').value.toUpperCase()}`;
 const ids=new Set();for(const p of s.players){if(p.id===id)continue;ids.add(p.id);if(!peers.has(p.id))peers.set(p.id,cowboy(p.color));const g=peers.get(p.id);g.userData.target=p;g.visible=p.hp>0;g.userData.hands.rotation.x=p.pitch;}
...
document.addEventListener('mousemove',e=>{if(!document.pointerLockElement)return;yaw-=e.movementX*.002;pitch=Math.max(-1.35,Math.min(1.35,pitch-e.movementY*.002));});
...
window.addEventListener('mousedown',e=>{if(e.button!==0||!document.pointerLockElement||local.hp<=0)return;const now=performance.now();if(now-lastShot<430||local.reloadUntil||reloading)return;if(!local.ammo){reload();return;}lastShot=now;recoil=.1;sound();flash.visible=true;setTimeout(()=>flash.visible=false,65);if(online)post('fire').catch(networkError);else{local.ammo--;$('ammo').textContent=local.ammo;const ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2(),camera);const hits=ray.intersectObjects(dummies,true);if(hits.length){$('hit').style.opacity=1;setTimeout(()=>$('hit').style.opacity=0,130);}shotEffect({origin:camera.position,direction:ray.ray.direction,distance:hits[0]?.distance||60});}});
setInterval(()=>{if(!online)return;const active=!!document.pointerLockElement;post('input',{x:active?Number(keys.has('KeyD'))-Number(keys.has('KeyA')):0,z:active?Number(keys.has('KeyS'))-Number(keys.has('KeyW')):0,yaw,pitch,jump:active&&keys.has('Space')}).catch(networkError);},50);
function frame(now){requestAnimationFrame(frame);const dt=Math.min((now-last)/1000,.05);last=now;const t=(now-started)/1000,locked=!!document.pointerLockElement;
 ...
 const moving=locked&&['KeyW','KeyA','KeyS','KeyD'].some(k=>keys.has(k)),bob=moving?Math.sin(t*9)*.025:Math.sin(t*1.8)*.004;
 camera.position.lerp(new THREE.Vector3(local.x,local.y+1.5+bob,local.z),online?1-Math.exp(-20*dt):1);recoil*=Math.exp(-13*dt);camera.rotation.set(pitch+recoil*.45,yaw,moving?Math.sin(t*4.5)*.008:0,'YXZ');rig.position.set(Math.sin(t*3)*.004,bob*.5,recoil);rig.rotation.x=recoil*2;
 ...
 for(let i=traces.length-1;i>=0;i--){const tr=traces[i];tr.life-=dt;if(tr.life<=0){scene.remove(tr.line);tr.line.geometry.dispose();tr.line.material.dispose();traces.splice(i,1);}}
