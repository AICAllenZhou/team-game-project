...
Press 2 for the side-by-side double-barrel shotgun (1 returns to the revolver). Left-click fires one barrel; right-click fires both loaded barrels together. A single-barrel shot emits 12 pellets in a 2.8-degree half-angle spread cone, followed by the other barrel on the next left click (24 pellets across both shots, or 24 in one right-click). Each shot consumes one shell and drops its own visible hammer; R starts a 2.4-second break-action reload. Reloading restores both shells and recocks the hammers. The shotgun retains independent free aim without ADS; its right button fires instead of entering focus. The revolver still uses RMB for slower camera movement. The shotgun has no aiming line or heat-wave beam; normal muzzle flash, bloom, recoil and shot vignette remain. Pellet directions and muzzle positions match client/server prediction, with one instanced draw for short, thin pellet tracers smaller than the revolver projectile. Weapon switching preserves each weapon's ammo and cannot interrupt reloads.
