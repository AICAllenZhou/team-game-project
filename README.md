...
A medium-paced, low-poly Wild West FPS built with HTML, CSS, JavaScript and Three.js. Includes a flat base plate, bean cowboys with sphere hands, low-poly revolvers, hip-fire only, a wide helmet/bodycam-style view, free-aim weapon inertia, six-round cylinders, 1.8-second reloads, health, three-second respawns and a scoreboard. The hidden free-aim point moves the hands and revolver independently inside a deadzone; once it reaches the edge, the helmet view turns. A fired revolver uses the actual delayed barrel direction. Movement and Q/E leaning add weapon motion. Movement is 4.5 units/second; three hits eliminate a player. No aiming-down-sights mode.

### Run multiplayer

Only the gun hand is visible, including on other players. Free aim allows 16 degrees to either side normally and 32 degrees with RMB, which also slows camera turning to 30% speed. Mouse input is applied per frame with smooth camera rotation; releasing RMB does not generate a camera jump. Local movement is predicted between server updates. The revolver's anchor remains 0.78 units from the eye; a spring-driven wrist lets its barrel kick upward and settle with a little overshoot. Shots originate at the visible muzzle and trigger a layered muzzle flash, warm light, and a brief edge vignette.
...
Shots launch a visible glowing round immediately. Your confirmed hits show a compact 28-pixel outlined screen marker at the projected impact location, including floor, fence, stand, target and player hits. The marker stays fixed in screen position and size, holds full brightness for 0.15 seconds, then fades out by 0.25 seconds. Sky misses have no marker. The visible flight uses elapsed time and lasts at least 50 ms for close shots; multiplayer confirms the impact before displaying its marker. The wrist has a stronger upward impulse with spring recovery.

Camera sensitivity is constant across the entire hand range, with RMB selecting a slower rate. Walking has a level camera, with bob applied only to the gun. Small server timing differences no longer pull the player back every update; larger position corrections fade in continuously across the tolerance boundary, are speed-limited, and are smoothed for display. Camera recoil uses a gentle spring impulse instead of an instantaneous angle jump, while the gun retains its strong kick. Input requests are serialized to avoid stale movement updates arriving out of order.
